...
**Личный второй мозг в Telegram: помнит вашу жизнь и советует, опираясь на неё.**

Вы наговариваете голосовое по дороге с встречи, пересылаете документ, пишете
в свой канал-дневник — бот раскладывает это по датам и людям, а когда вы
приходите с сомнением («стоит ли соглашаться?»), поднимает ваши прошлые
похожие решения, смотрит, чем они кончились, и отвечает прогнозом с
обоснованием.

База накапливается в обычном **Obsidian-хранилище**: markdown-файлы с
`[[перекрёстными ссылками]]`, отдельный файл на каждого человека. Она читается
без бота и остаётся вашей, даже если бот завтра выключится.

### Чем это отличается от чат-бота

| Чат-бот | second_brain |
|---|---|
| Помнит текущий диалог | Помнит всё, что вы когда-либо ему рассказали |
| Советует «вообще» | Советует, опираясь на **ваши** прошлые решения и их исходы |
| История — в чужом облаке, в чужом формате | База — markdown в вашем Obsidian, вы её владелец |
| Одна модель за фиксированный прайс | Работает на топ-1 бесплатной модели OpenRouter, платная — только там, где слой критичен |

### Как это выглядит

> **Вы:** *(голосовое)* Думаю, не согласиться ли на предложение Марка по франшизе.
> Смущает, что опять всё быстро.
>
...
Прогноз собирается не одним запросом, а конвейером слоёв: суть решения →
факторы и риски → прецеденты из вашей базы → взгляд оптимиста/пессимиста/
реалиста → синтез консультанта (`ТЗ.md` §9, промты — `Promts/`).
...
| Консультант: прецеденты из вашей базы → прогноз с обоснованием; исход решения дописывается позже и питает будущие прогнозы | работает |
| Канал-дневник: бот вычитывает **всю прошлую историю** канала/группы, а не только новые посты (или импортирует JSON-выгрузку Telegram Desktop, если источник приватный) | работает |
| Обратная связь на ответ: разбор «что не так» пишется в базу вместе с именем модели, модель можно сменить | работает (текстовой реакцией: inline-кнопки 👍👎 в hermes не подтверждены — `ТЗ.md` §13.5) |
...
| Фото с людьми, рукописный текст | по плану (M4–M5, `DEV_PLAN.md`) |

Полный список сценариев — [`PRD.md`](./PRD.md) §2.3, статус разработки —
[`TODO.md`](./TODO.md) и [`DEV_PLAN.md`](./DEV_PLAN.md).

### Кому это

Одному человеку — владельцу. Бот персональный: отвечает только вам
(`TELEGRAM_ALLOWED_USERS`), в канале игнорирует посты других админов.
Мультитенантность — явная не-цель (`PRD.md` §2.2).

### Что нужно, чтобы поднять

Telegram-бот, ключ **OpenRouter** (LLM-«мозг») и ключи **Yandex Cloud**
(голос + эмбеддинги памяти). Разворачивается одним контейнером на
**amvera.ru** — [быстрый старт](#быстрый-старт-локально),
[деплой](#деплой-на-amveraru), [ключи](#ключи-где-взять).
